...
tool = pd.read_csv(TOOL_PATH, sep="\t")
...
axes[0].set_xlabel("Number of pixels ($10^8$)", fontsize=10)
axes[0].set_ylabel("Time (s)", fontsize=10)
axes[0].ticklabel_format(style="plain", axis="x")

sns.barplot(tool, x="Method", y="Time", order=ORDER, ax=axes[1])
axes[1].set(yscale="log")
axes[1].tick_params(axis="both", labelsize=8)
axes[1].set_xlabel("Tool", fontsize=10)
axes[1].set_ylabel("Time (s)", fontsize=10)
plt.xticks(rotation=30)

fig.text(0.04, 0.91, "A)", ha="center", fontsize=16)
fig.text(0.45, 0.91, "B)", ha="center", fontsize=16)
